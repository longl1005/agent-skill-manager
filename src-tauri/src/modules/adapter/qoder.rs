...
impl AgentAdapter for QoderAdapter {
    fn id(&self) -> AgentId { AgentId("qoder".to_string()) }

    fn descriptor(&self) -> AgentDescriptor {
        AgentDescriptor {
            agent_id: self.id(),
            adapter_id: "qoder@1".to_string(),
            display_name: "Qoder".to_string(),
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            documentation_url: Some("https://docs.qoder.com/en/cli/Skills".to_string()),
    ...
    fn capabilities(&self) -> CapabilitySet {
        CapabilitySet {
            detect: SupportLevel::Supported, scan: SupportLevel::Supported,
            compare_content: CompareConfidence::Reliable,
            watch: SupportLevel::Unsupported, install_planning: SupportLevel::Unsupported,
            uninstall_planning: SupportLevel::Unsupported, update_planning: SupportLevel::Unsupported,
            sync_planning: SupportLevel::Unsupported,
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            notes: vec!["user-scope $HOME/.qoder/skills".to_string()],
        }
    }
    ...
    fn detect(&self, ctx: &DetectContext) -> DetectionResult {
        ...
        let root = ctx.custom_path.map(|path| path.to_path_buf()).unwrap_or_else(|| {
            ...
            home.join(".qoder").join("skills")
        });
        let exists = std::fs::symlink_metadata(&root).map(|metadata| metadata.file_type().is_dir()).unwrap_or(false);
        DetectionResult {
            agent: self.id(), status: if exists { DetectionStatus::Detected } else { DetectionStatus::Unavailable },
    ...
    fn scan(&self, ctx: &ScanContext) -> ScanResult {
        ...
        for installation in &mut result.installations {
            ...
            installation.adapter_id = "qoder@1".to_string();
        }
        result
    }
}
