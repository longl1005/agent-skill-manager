...
impl AgentAdapter for AntigravityAdapter {
    fn id(&self) -> AgentId {
        AgentId("antigravity".to_string())
    }

    fn descriptor(&self) -> AgentDescriptor {
        AgentDescriptor {
            agent_id: self.id(),
            adapter_id: "antigravity@1".to_string(),
            display_name: "Antigravity".to_string(),
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            documentation_url: Some("https://antigravity.google.com".to_string()),
    ...
    fn capabilities(&self) -> CapabilitySet {
        CapabilitySet {
            ...
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            notes: vec!["user-scope $HOME/.gemini/antigravity/builtin/skills".to_string()],
        }
    }
    ...
    fn detect(&self, ctx: &DetectContext) -> DetectionResult {
        ...
        if matches!(status, DetectionStatus::Unavailable) {
            issues.push(ScanIssue {
                ...
                phase: IssuePhase::Detect,
                path: None,
                message: "no Antigravity Skills roots found".into(),
                recoverable: true,
            });
        }
...
fn build_installation_full(
    ...
    SkillInstallation {
        ...
        format: SkillFormatDescriptor {
            id: "antigravity-skill".into(),
            display_name: "Antigravity Skill".into(),
...
mod tests {
    ...
    fn tempdir() -> PathBuf {
        ...
        let nanos = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("asm-ag-{}-{}-{}", pid, nanos, n));
    ...
    #[test]
    fn antigravity_detect_only_user() {
        let tmp = tempdir();
        let home = tmp.join("home");
        let skills = home.join(".gemini/antigravity/skills");
