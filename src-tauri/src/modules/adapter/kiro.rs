...
/// Adapter for Kiro CLI's global skills.
pub struct KiroAdapter;

impl AgentAdapter for KiroAdapter {
    fn id(&self) -> AgentId {
        AgentId("kiro".to_string())
    }

    fn descriptor(&self) -> AgentDescriptor {
        AgentDescriptor {
            agent_id: self.id(),
            adapter_id: "kiro@1".to_string(),
            display_name: "Kiro CLI".to_string(),
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            documentation_url: Some("https://kiro.dev/docs/cli/skills/".to_string()),
    ...
    fn capabilities(&self) -> CapabilitySet {
        CapabilitySet {
            ...
            supported_platforms: vec![Platform::MacOs, Platform::Linux, Platform::Windows],
            notes: vec!["user-scope $HOME/.kiro/skills".to_string()],
        }
    }
    ...
    fn detect(&self, ctx: &DetectContext) -> DetectionResult {
        ...
        let root = ctx.platform.home_dir.join(".kiro").join("skills");
        if path_exists(&root) {
            ...
            DetectionResult {
                ...
                issues: vec![ScanIssue {
                    ...
                    phase: IssuePhase::Detect,
                    path: None,
                    message: "no Kiro CLI Skills roots found".into(),
                    recoverable: true,
                }],
                observed_at,
            }
    ...
    fn scan(&self, ctx: &ScanContext) -> ScanResult {
        ...
        for installation in &mut result.installations {
            ...
            installation.adapter_id = "kiro@1".to_string();
            installation.format.id = "kiro-skill".to_string();
            installation.format.display_name = "Kiro CLI Skill".to_string();
        }
        result
    }
}
